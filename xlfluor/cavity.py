...
class Cavity:
    ...
    def layer_list_to_parameters(layer_list, d_tolerance=1, rho_tolerance=0.5e3):
        """
        ...
        for n, layer in enumerate(layer_list):
            ...
            pars.add(prefix + 'd', value=layer.d, min=layer.d - d_tolerance, max=layer.d + d_tolerance)
            pars.add(prefix + 'rho', value=layer.density, min=layer.density - rho_tolerance,
...
class CavitySolution:
    ...
    def __init__(self, cavity, problem):
        ...
        self.L_matrices_out_partials_inverse = np.zeros(
            (len(problem.energies_in), len(problem.angles_in), len(problem.z_axis),2,2), dtype=complex)  #L1^-1
        self.L_matrices_out_partials_reverse = np.empty(
            (len(problem.energies_in), len(problem.angles_in), len(problem.z_axis),2,2), dtype=complex)*np.nan  #Lz or L(D-z_p)
        ...
        self.layer_solutions.append(cavity.layer_list[-1].solve(self.problem, final=True))
    ...
    def _solve_layers(self, parameters):
        ...
        :return:
        """
        for n, layer in self.problem.cavity.layer_list:
            prefix = f'Layer_{n}_({layer.material.name})_'
            d = parameters[prefix + 'd']
            rho = parameters[prefix + 'rho']

            self.layer_solutions[n] = layer.solve(self.problem, d, rho,
                                                  final=(n == (self.problem.cavity.N_layers - 1)))
    ...
    def _calc_L_total(self):
        ...
        for n, layer in self.cavity.layer_list:
    ...
    def _calc_L_partial(self):
        ...
        for iz, z in self.problem.z_axis:
            ...
            for n, layer in self.cavity.layer_list[:partial_layer_index]:
            ...
            iz_within_layer = iz - np.where(self.problem.z_layer_indices == partial_layer)[0][0] # where statement finds the first z-index of the active layer
            ...
            if partial_layer.is_active:
                self.L_matrices_out_partials[:,:,iz_within_layer,:,:] = partial_layer.solution.L_matrices_out[:,:,iz_within_layer,:,:] @ self.L_matrices_out_partials[:,:,iz,:,:]
    ...
    def _calc_fluorescence(self):
        ...
        for n, layer in enumerate(self.cavity.layer):
            if not layer.is_active:
                continue
            relevant_z_indices = np.where(self.problem.z_layer_indices == n)
            ...
            excitation_intensity = xlfluor.functions.abs2(self.incident_field_amplitude[:,:,relevant_z_indices])       # Ein,Ain,z(layer)
            ...
            way_to_the_surface = (L1i[:,:,relevant_z_indices,1, 1] - (L1i[:,:,relevant_z_indices,1, 0] * L1i[:,:,relevant_z_indices,0, 1]) / L1i[:,:,relevant_z_indices,0, 0]) # Eout,Aout, z(layer)

            for iEin, Ein in self.problem.energies_in:
                for iAin, Ain in self.problem.angles_in:
                    self.fluorescence_local_amplitude[iEin, :,iAin ,:, relevant_z_indices, 0] = (fluorescence_amplitude[iEin, iAin, :] * R1[...]) / (1 - R2[...] * R1[...])  # A+ (down)
                    self.fluorescence_local_amplitude[iEin, :,iAin ,:, relevant_z_indices, 1] = (fluorescence_amplitude[iEin, iAin, :] * R2[...]) / (1 - R1[...] * R2[...])  # A- (up)
